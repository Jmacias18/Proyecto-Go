...
import "../Styles/AdminPasajero.css";

function AdminPasajero() {
    ...
    const handleSave = (event) => {
        ...
        axiosInstance.put(`/pasajeros/${id}`, dataToSend)
            .then(response => {
    ...
    const handleDelete = (id) => {
        ...
        axiosInstance.put(`/pasajeros/${id}`, { activo: 0 })
            .then(response => {
    ...
    const handleSubmit = async (event) => {
        ...
        nuevoPerfil.rol = 1; // Asegurarse de que el rol sea 'pasajero'
        nuevoPerfil.activo = 1; // Asegurarse de que el pasajero esté activo
        ...
        try {
            const responseCorreo = await axiosInstance.get(`/pasajeros?correo=${nuevoPerfil.correo}`);
        ...
        try {
            const responseTelefono = await axiosInstance.get(`/pasajeros?telefono=${nuevoPerfil.telefono}`);
        ...
        // Enviar los datos al servidor para crear un nuevo pasajero
        axiosInstance.post('/pasajeros', nuevoPerfil)
            .then(response => {
                console.log('Perfil creado:', response.data); // Log de la respuesta del servidor
                // Actualizar la lista de usuarios con el nuevo pasajero
        ...
        <div className="container">
                    ...
                    <tbody>
                        {filteredUsuarios.map((usuario) => (
                            <tr key={usuario.id}>
                                ...
                                <td>{usuario.rol === 1 ? 'Pasajero' : 'Otro Rol'}</td>
                                <td>
                                    <FaPen
                                        className="icon"
            ...
            {isModalOpen && (
                <div className="modal-overlay">
                    <div className="modal">
                        <h2>{editingUser && editingUser.id ? 'Editar Usuario' : 'Agregar Nuevo Perfil'}</h2>
